...
// netlify/functions/admin-projects.ts
var admin_projects_exports = {};
__export(admin_projects_exports, {
  handler: () => handler
});
module.exports = __toCommonJS(admin_projects_exports);
...
var handler = async (event) => {
  ...
  try {
    if (body.action === "delete" && body.id) {
      const { error } = await supabase.from("portfolio_projects").delete().eq("id", body.id);
